...
2. **`core-team` ist der einzig real existente Review-Channel.** Alle 17 Domain-Team-Slugs (`@OpenSIN-AI/messaging`, `@OpenSIN-AI/apple`, …) die historische Versionen der Canon-Docs und `templates/teams/*.json` in `codeowners` / `review_teams` Feldern referenzieren **existieren nicht als GitHub Teams** (`GET /orgs/OpenSIN-AI/teams` liefert 2 Einträge: `core-team`, `admin-team`). Konsequenz: CODEOWNERS-Einträge mit diesen Slugs werden stillschweigend ignoriert. `.github/CODEOWNERS` wurde am 2026-04-19 auf `@OpenSIN-AI/core-team` korrigiert (commit in diesem PR). Tracked als **MAN-4** — entweder die 17 Teams **auf GitHub anlegen**, oder die Manifest-Felder auf `core-team` vereinheitlichen.
